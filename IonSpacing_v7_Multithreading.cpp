...
const int N = 300;	//Number of Ions
...
int main() {
	...
	for (int i = 0; i < N; i++) {
		std::cout << std::fixed << std::setprecision(15) << "u[" << i+1 << "] = " << u_guess[i] << ", ";
	}
...
double uj_calc(int j, double u_old[]) {
	...
	#pragma omp parallel for reduction(+:first_sum)
	for (int i = 0; i < j; i++) {
		first_sum += (1 / pow(u_old[j] - u_old[i], 2));
	}

	#pragma omp parallel for reduction(+:sec_sum)
	for (int i = j + 1; i < N; i++) {
...
/*
Title: guessing
Description: Calculate for convergence
*/
...
bool guessing(double u_guess[], double& alpha, double u_guess_backup[], double& residual_s) {
	...
	#pragma omp parallel for reduction(+:residual_calc)
	for (int i = 0; i < (N / 2); i++) {
